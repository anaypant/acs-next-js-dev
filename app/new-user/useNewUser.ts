...
export function useNewUser() {
    ...
    const fetchUserData = useCallback(async () => {
        ...
        if (error || !data?.items?.[0]) {
            ...
            setEmailData(prev => ({ ...prev, responseEmail: user.acsMail || '' }));
            setLcpSettings({
    ...
    const handleProfileSubmit = async () => {
        ...
        const { success, error } = await update({
            ...
            key_name: 'id',
            key_value: userId,
            update_data: { ...profileData, job_title: profileData.jobTitle }
        });
        if (success) nextStep();
        else setError(error || 'Failed to save profile');
        setLoading(false);
    };
    ...
    const handleEmailSubmit = async () => {
        setLoading(true);
        const update_data = {
            responseEmail: emailData.emailOption === 'default' ? emailData.responseEmail : emailData.customEmail,
        ...
        if (success) nextStep();
        else setError(error || 'Failed to save email settings');
        setLoading(false);
    };
    ...
    const handleLcpSettingsSubmit = async () => {
        ...
        if (success) nextStep();
        else setError(error || 'Failed to save LCP settings');
        setLoading(false);
    }
    ...
    const handleSettingsSubmit = async () => {
        ...
        if (success) nextStep();
        else setError(error || 'Failed to save settings');
        setLoading(false);
    };
    ...
    const handleCompleteSetup = async () => {
        ...
        else setError(error || 'Failed to complete setup');
        setLoading(false);
    };
