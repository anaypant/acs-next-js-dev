...
export default function NewUserPage() {
    ...
    const verifyIdentity = async () => {
      ...
      setTxtRecord(null);
      try {
        const response = await apiClient.verifyDomain(customDomain);
        if (response.success && response.data?.txt_record) {
          setIdentityStatus(response.data.message || 'Identity verification initiated.');
          setTxtRecord(response.data.txt_record);
          setIdentityVerified(false); // User must confirm after adding TXT
        } else {
          const errorMessage = response.error || 'Failed to initiate identity verification.';
    ...
    const verifyDkim = async () => {
      ...
      setDkimStatus(null);
      try {
        const response = await apiClient.verifyDKIM(customDomain);
        
        if (response.success && response.data?.dkim_records) {
          ...
          setTxtRecord({
            Name: 'DKIM Records',
            Type: 'CNAME',
            Value: JSON.stringify(response.data.dkim_records, null, 2)
          });
        } else {
          const errorMessage = response.error || 'Failed to initiate DKIM verification.';
          setDkimVerified(false);
      ...
      const fetchData = async () => {
        try {
          ...
          if (!defaultEmail) {
            ...
            const checkResponse = await apiClient.dbSelect({
                table_name: 'Users',
                index_name: 'responseEmail-index',
                key_name: 'responseEmail',
            ...
            if (!checkResponse.success) {
              throw new Error(checkResponse.error || 'Failed to check email availability');
            }
    ...
    const handleCustomizeEmail = async () => {
      try {
        ...
        if (emailOption === 'custom' && session?.user?.id) {
          // Only verify email validity and update for custom domain
          const verifyResponse = await apiClient.verifyEmailValidity(session.user.id, finalEmail);

          if (!verifyResponse.success) {
            throw new Error(verifyResponse.error || 'Failed to verify email validity');
          }

          // For custom domain, only update responseEmail
          const updateRes = await apiClient.dbUpdate({
              table_name: 'Users',
              index_name: 'id-index',
              key_name: 'id',
              key_value: session?.user?.id,
          ...
          if (!updateRes.success) {
            throw new Error(updateRes.error || 'Failed to update response email');
          }
          ...
          if (!updateRes.success) {
            throw new Error(updateRes.error || 'Failed to update ACS Mail and response email');
          }
        }
        ...
        if (!updateRes.success) {
          throw new Error(updateRes.error || 'Failed to update email and LCP settings');
        }
    ...
    const handleProfileSubmit = async () => {
      try {
        ...
        if (!updateRes.success) {
          throw new Error(updateRes.error || 'Failed to save profile information');
        }
    ...
    const handleCompleteSetup = async () => {
      try {
        ...
        if (!updateRes.success) {
          throw new Error(updateRes.error || 'Failed to update LCP settings');
        }
    ...
    if (loading && step === 1) {
        return <LoadingSpinner text="Loading your information..." size="lg" />;
    }
